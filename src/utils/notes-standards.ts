...
const POSSESSIVE_PATTERN = new RegExp(`^(.{0,6})?(.+?(?:${CORE_CONCEPTS.join('|')}))(.{0,8})?$`);
...
const TITLE_SUFFIX_RE =
  /(的研究|的发现|的分析|的影响|的问题|的方法|的策略|的机制|的效果|的报告|的调查)$/gi;
...
export function validateAtomicNote(note: AtomicNote): { valid: boolean; issues: string[] } {
  ...
  const hasHardIssue =
    note.content.length < MIN_NOTE_CONTENT_LENGTH || !note.title || note.title.trim() === '';
...
function tryParseFrontmatterFallback(text: string): AtomicNote[] {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (hasMetadata) {
      ...
      const _metaEndIndex = lines.length;
...
function extractTagCandidates(content: string, title?: string): string[] {
  ...
  if (keywords.size < 2) {
    ...
    const subjectMatch = firstSentence.match(
      /^(.{2,12})(?:是|指|通过|利用|基于|采用|包括|涉及|表现为|被称为)/,
    );
    if (subjectMatch) {
